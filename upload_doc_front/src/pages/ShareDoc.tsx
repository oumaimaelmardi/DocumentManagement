...
function ShareDoc({open, handleClose, document}: DocumentsDetailsProps) {
    ...
    const handleShare = async () => {
            if (document) {
                ...
                try {
                    const response = await fetch('http://localhost:8081/api/acces/add', {
                        method: 'POST',
                        headers: {
                    ...
                    if (response.ok) {
                        setResponseMessage('Access added successfully!');
